...
from login_register.registerfunc import register
from common.my_logger import my_logger
@ddt
class LoginTest(unittest.TestCase):
    excel=ReadExcel(r'\python24_test_project\data\case.xlsx','login')
    ...
    def test_login(self,case):
        ...
        try:
            ...
            my_logger.info('测试用例:{}-->>执行未通过'.format(case['title']))
            my_logger.error(e)
            raise e
        else:
            self.excel.write_excel(row=case_id+1, column=5, value='已通过')
            my_logger.info('测试用例:{}-->>执行已通过'.format(case['title']))

@ddt
class RegisterTest(unittest.TestCase):
    excel=ReadExcel(r'\python24_test_project\data\case.xlsx','register')
    case_data=excel.read_excel()
